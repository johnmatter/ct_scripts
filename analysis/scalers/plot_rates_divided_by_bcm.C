...
void plot_rates_divided_by_bcm() {
    ...
    for (auto const &k : kinematics) {
        ...
        // pTRIG1
        histoName = Form("h_%s_ptrig", k.Data());
        drawMe = Form("(P.pTRIG1.scalerRate/P.BCM4A.scalerCurrent):This->GetReadEntry()>>%s", histoName.Data());
        scalerTree->Draw(drawMe.Data());
        histoTemp = (TH2F*) gPad->GetPrimitive(histoName.Data());
        histoTitle = Form("%s", k.Data());
        xAxisTitle = Form("Entry");
        yAxisTitle = Form("P.pTRIG1.scalerRate/P.BCM4A.scalerCurrent");
        ...
        // hTRIG1
        histoName = Form("h_%s_htrig1", k.Data());
        drawMe = Form("(P.hTRIG1.scalerRate/P.BCM4A.scalerCurrent):This->GetReadEntry()>>%s", histoName.Data());
        scalerTree->Draw(drawMe.Data());
        histoTemp = (TH2F*) gPad->GetPrimitive(histoName.Data());
        histoTitle = Form("%s", k.Data());
        xAxisTitle = Form("Entry");
        yAxisTitle = Form("P.hTRIG1.scalerRate/P.BCM4A.scalerCurrent");
        histoTemp->SetTitle(histoTitle.Data());
        histoTemp->GetXaxis()->SetTitle(xAxisTitle.Data());
        histoTemp->GetYaxis()->SetTitle(yAxisTitle.Data());
        cRates->Update();
