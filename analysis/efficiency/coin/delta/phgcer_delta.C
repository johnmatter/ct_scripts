...
void phgcer_delta() {
    ...
    TCut cutDid = cuts->Get("pHGCerDid");
    ...
    cPcer->Print("phgcer_delta.pdf["); // open PDF
    cPcer->Divide(2,2);
    int pad=1;
    TString t = "LH2";
    for (auto const &q : Q2s) {
    ...
    cPcer->Print("phgcer_delta.pdf"); // write page to PDF
    ...
    cPcer->Print("phgcer_delta.pdf]"); // close fPDF
}
