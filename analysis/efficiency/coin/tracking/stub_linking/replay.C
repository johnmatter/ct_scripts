void replay(Int_t RunNumber = 0, Int_t MaxEvent = 0, Double_t xt=0, Double_t xpt=0, Double_t y=0, Double_t ypt=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/coin_replay_production_%d_%d_xt%.3f_xpt%.3f_yt%.3f_ypt%.3f.root";
  ...
  TString paramDir = "/home/jmatter/ct_scripts/analysis/efficiency/coin/tracking/stub_linking/PARAM";
  ...
  //Adding December data method
  // THaDecData* decdata= new THaDecData("D","Decoder raw data");
  // gHaApps->Add(decdata);

  // Define the analysis parameters
  TString ROOTFileName = Form(ROOTFileNamePattern, RunNumber, MaxEvent, xt, xpt, y, ypt);
  ...
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/COIN/PRODUCTION/summary_production_%d_%d_xt%.3f_xpt%.3f_yt%.3f_ypt%.3f.report", RunNumber, MaxEvent, xt, xpt, y, ypt));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production.template",
  			Form("REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_production_%d_%d_xt%.3f_xpt%.3f_yt%.3f_ypt%.3f.report", RunNumber, MaxEvent, xt, xpt, y, ypt));  // optional
