void replay(Int_t RunNumber = 0, Int_t MaxEvent = 0, Double_t spacepoint=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/coin_replay_production_%d_%d_spacepoint%.3f.root";
  ...
  TString ROOTFileName = Form(ROOTFileNamePattern, RunNumber, MaxEvent, spacepoint);
  ...
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/COIN/PRODUCTION/summary_production_%d_%d_spacepoint%.3f.report", RunNumber, MaxEvent, spacepoint));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production.template",
  			Form("REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_production_%d_%d_spacepoint%.3f.report", RunNumber, MaxEvent, spacepoint));  // optional
