...
// Investigating what spacepoints look like for the old and new rootfiles
void plot_nsp() {
    ...
    CTData *dataNew = new CTData("/home/jmatter/ct_scripts/ct_coin_data_pass2_newtrack.json");
    CTCuts *cutsJson = new CTCuts("/home/jmatter/ct_scripts/cuts.json");

    // PDF to print to
    TString pdfFilename = "/home/jmatter/ct_scripts/analysis/efficiency/coin/distributions/distributions_nsp.pdf";
    ...
    b = "P.dc.nsp";
    nBins[b]=40; loBin[b]=0; hiBin[b]=20;
    ...
    for (auto const &k : kinematics) {
        for (auto const &branch : branches) {
            ...
            for (auto const &cut : cuts) {
                ...
                // write logy plot for good measure
                c->SetLogy(1);
                c->Print(pdfFilename.Data());
                c->SetLogy(0);
