...
void pcal_plateau_singles() {
    ...
    cPcal->Print("pcal_plateau_singles.pdf["); // open PDF

    TMultiGraph *mgLH2 = new TMultiGraph("mgLH2", "Efficiency");
    TString t = "LH2";
    Int_t n=0; // color
    TLegend *legLH2= new TLegend(0.6,0.1,0.9,0.3);
    for (auto const &q : Q2s) {
        // Make it pretty
        std::pair<TString, Int_t> key = std::make_pair(t,q);
    ...
    mgLH2->Draw("ALP");
    mgLH2->GetXaxis()->SetTitle("P.cal.eprtracknorm cut");
    mgLH2->GetYaxis()->SetTitle("Efficiency");
    legLH2->Draw();
    cPcal->Print("pcal_plateau_singles.pdf"); // write page to PDF

    TMultiGraph *mgC12 = new TMultiGraph("mgC12", "Efficiency");
    t = "C12";
    n=0; // color
    TLegend *legC12= new TLegend(0.6,0.1,0.9,0.3);
    for (auto const &q : Q2s) {
        // Make it pretty
        std::pair<TString, Int_t> key = std::make_pair(t,q);
        efficiencyGraphs[key]->SetMarkerStyle(21);
    ...
    mgC12->Draw("ALP");
    mgC12->GetXaxis()->SetTitle("P.cal.eprtrack cut");
    mgC12->GetYaxis()->SetTitle("Efficiency");
    legC12->Draw();
    cPcal->Print("pcal_plateau_singles.pdf"); // write page to PDF

    cPcal->Print("pcal_plateau_singles.pdf]"); // close fPDF
}
