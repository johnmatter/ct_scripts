...
for f in `ls $runlist_dir/runs_*Q2_*`; do
    f_base=`basename $f`
    $hcswif_dir/hcswif.py --mode replay --spectrometer HMS_COIN --events -1 --project c-comm2017 --run file $f --name hms_$f_base --disk 1000000000
    $hcswif_dir/hcswif.py --mode replay --spectrometer SHMS_COIN --events 500000 --project c-comm2017 --run file $f --name shms_$f_base --disk 1000000000
done
